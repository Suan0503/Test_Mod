	...
	def render(self, template, **kwargs):
		if 'extra_actions' not in kwargs:
			kwargs['extra_actions'] = [
				{'label': '新增白名單', 'url': '/admin_panel/whitelist/new', 'icon': 'fa fa-user-plus'},
				{'label': '新增黑名單', 'url': '/admin_panel/blacklist/new', 'icon': 'fa fa-user-times'},
			]
		return super().render(template, **kwargs)

def init_admin(app):
	admin = Admin(app, name='後台管理', template_mode='bootstrap4', base_template='admin_custom_master.html', url='/admin_panel')
